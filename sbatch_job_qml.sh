...
if [ "$python_version" = "$required_version" ]; then
    ...
    rnd_seed=$(expr $SLURM_ARRAY_TASK_ID / 16)
    model_number=$(expr $SLURM_ARRAY_TASK_ID % 16)
    if [ $(expr $model_number / 4) -eq 0 ]; then
        model_class=QuantumAngle2PCGNN
    elif [ $(expr $model_number / 4) -eq 1 ]; then
        model_class=QuantumElementwiseAngle2PCGNN
    elif [ $(expr $model_number / 4) -eq 2 ]; then
        model_class=QuantumIQP2PCGNN
    elif [ $(expr $model_number / 4) -eq 3 ]; then
    ...
    python a.py --model_class $model_class --gnn_layers $gnn_layers --gnn_reupload $gnn_reupload --rnd_seed $rnd_seed
else
